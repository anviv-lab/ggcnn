...
def plot_output(rgb_img, depth_img, grasp_position_img, grasp_angle_img, ground_truth_bbs, no_grasps=1, grasp_width_img=None, idx=0):
        """
        ...
        gs = detect_grasps(grasp_position_img, grasp_angle_img, width_img=None, no_grasps=no_grasps, ang_threshold=0)

        fig = plt.figure(figsize=(10, 10))
        ax = fig.add_subplot(2, 2, 1)
        ax.imshow(rgb_img)

        folder = "../ggcnn_results/"
        ...
        for i, g in enumerate(gs):
            g.plot(ax)
            output[i] = {
                "center": [int(g.center[1]), int(g.center[0])],
                "angle": -float(g.angle),
                "width": float(g.width),
            }

        with open(folder + "grasp_result_" + str(idx) + ".json", "w") as f:
            json.dump(output, f)
        ...
        plt.colorbar(plot)
        plt.savefig(folder + "grasp_result_" + str(idx) + ".png")

        # save the images
        out_img = np.concatenate([rgb_img, np.expand_dims(depth_img, axis=-1)], axis=-1)
        np.save(folder + "rgbd_{}.npy".format(idx) ,out_img)
...
def run():
    ...
    for model_folder in model_folders:
        ...
        with open(os.path.join(model_folder, '_dataset.txt')) as f:
            ...
            dataset_fn = "data/datasets/crop_norotate_normalise.hdf5"
        ...
        for epoch in epochs:
            ...
            if os.path.exists(model_output_fn):
                ...
                input_data = np.expand_dims(depth_imgs, axis=-1)
                

                for i in range(depth_imgs.shape[0]):

                    model_output_data = model.predict(np.expand_dims(input_data[i], axis=0))
                    ...
                    plot_output(rgb_imgs[i, ].squeeze(), depth_imgs[i, ].squeeze(), grasp_positions_out.squeeze(), grasp_angles_out.squeeze(), bbs_all,
                                no_grasps=NO_GRASPS, grasp_width_img=grasp_width_out.squeeze(), idx=i)
